pipeline {
    ...
    stages{
        stage('git cloned'){
            steps{
                git url:'https://github.com/shrikant-devops/php-project/', branch: "master"
          ...
          stage('Docker login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-pwd', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push shrikantdevops/phpprojectv1'
                }
            }
        }
            ...
            steps {
               script {
                   def dockerrm = 'sudo docker rm -f My-first-containe2211 || true'
                    def dockerCmd = 'sudo docker run -itd --name My-first-containe2211 -p 8083:80 shrikantdevops/phpprojectv1'
                    sshagent(['sshkeypair']) {
